...
function buildSearchQuery(track) {
  return `ytsearch1:${track.artist} ${track.title} audio`.replace(/"/g, '');
}
...
function downloadOne(track, savePath, options, onLog) {
  return new Promise(async (resolve, reject) => {
    ...
    ffArgs.push('-f', cfg.ext === 'm4a' ? 'ipod' : cfg.ext, outPath);
    ...
    yt.on('close', code => {
      ...
      if (code !== 0 && code !== null && onLog) onLog(`yt-dlp exit ${code}: ${ytErr.trim().slice(-300)}`);
      finish(null);
    });
    ff.on('close', code => {
      if (code !== 0) return finish(new Error(`ffmpeg failed: ${ffErr.trim().slice(-400) || 'unknown'}`));
      finish(null);
    });
  });
}
